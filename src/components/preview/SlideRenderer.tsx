...
export function SlideRenderer({ slide, width = 1080, height = 1350, className, isExport, selectedElement, onElementClick }: Props) {
  ...
  const highlightSize = isExport ? `${(typo.highlightSize ?? (typo.bodySize ?? 16)) * 1.3}px` : `${typo.highlightSize ?? (typo.bodySize ?? 16)}px`;
  const bodySize = isExport ? `${(typo.bodySize ?? 16) * 1.5}px` : `${typo.bodySize ?? 16}px`;
  const bulletSize = isExport ? `${(typo.bulletSize ?? (typo.bodySize ?? 16) - 1) * 1.4}px` : `${typo.bulletSize ?? ((typo.bodySize ?? 16) - 1)}px`;
  const catSize = isExport ? `${(typo.categorySize ?? 12) * 1.3}px` : `${typo.categorySize ?? 12}px`;
  const ctaSize = isExport ? `${(typo.ctaSize ?? (typo.bodySize ?? 16)) * 1.5}px` : `${typo.ctaSize ?? (typo.bodySize ?? 16)}px`;
  const sourceLabelSize = isExport ? `${(typo.sourceLabelSize ?? 11) * 1.3}px` : `${typo.sourceLabelSize ?? 11}px`;
